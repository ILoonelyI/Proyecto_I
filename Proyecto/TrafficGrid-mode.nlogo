...
to set-signals
  if semaforos-mode = "coordinados" [
    ;; Semáforos coordinados: comportamiento original
    ask intersections with [auto? and phase = floor ((my-phase * ticks-per-cycle) / 100)] [
      set green-light-up? (not green-light-up?)
      set-signal-colors
    ]
  ]
  if semaforos-mode = "aleatorio" [
    ;; Semáforos con luces verdes aleatorias
    ask intersections with [auto? and phase = floor ((my-phase * ticks-per-cycle) / 100)] [
      if random 100 < 50 [  ;; Probabilidad del 50% de cambiar a verde
        set green-light-up? (not green-light-up?)
        set-signal-colors
      ]
    ]
...
semaforos-mode
semaforos-mode
"coordinados" "aleatorio" "secuencial"
1
...
@#$#@#$#@
<experiments>
  <experiment name="analisis" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="20000"/>
    <metric>num-cars-stopped</metric>
    <metric>mean [speed] of turtles</metric>
    <metric>mean [wait-time] of turtles</metric>
    <runMetricsCondition>ticks mod 100 = 0</runMetricsCondition>
    <steppedValueSet variable="grid-size-y" first="4" step="3" last="10"/>
    <enumeratedValueSet variable="power?">
      <value value="true"/>
      <value value="false"/>
    </enumeratedValueSet>
    <steppedValueSet variable="grid-size-x" first="4" step="3" last="10"/>
    ...
    <enumeratedValueSet variable="semaforos-mode">
      <value value="&quot;aleatorio&quot;"/>
      <value value="&quot;coordinados&quot;"/>
    </enumeratedValueSet>
    <steppedValueSet variable="ticks-per-cycle" first="20" step="20" last="100"/>
